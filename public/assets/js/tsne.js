...
/**
 * ImageDeets: polyline user-selections
 **/
...
Picker.prototype.onMouseHover = function(e){
...
Picker.prototype.onMouseMove = function(e) {
  ...
  // if the click wasn't on the canvas, quit
  var cellIdx = this.select({x: click.x, y: click.y});
  var titleCell = document.querySelector('#title-container')
  ...
  if(entered === 1){
    ...
    var poy = e.clientY + 10;
    var vid_name = filename.slice(0, -4)
    var title_text = vid_name.replace(/([A-Z])/g, ' $1').trim()
    titleCell.style.left = pox + 'px';
    titleCell.style.top = poy + 'px';
    titleCell.innerHTML = title_text;
...
var vid_name = filename.slice(0, -4)
  ...
  function showModal(json) {
    ...
    //var vid = document.querySelectorAll('#yt-vid')[0].src;
    var title = document.querySelectorAll('#image-title');

  

    var title_text = vid_name.replace(/([A-Z])/g, ' $1').trim()
    title[0].innerHTML = title_text
  ...
  if (this.isExecuted == false) {
	...
	// load correct video source from youtube. 
...
function LOD() {
  ...
  this.minZ = 1; // minimum zoom level to update textures
...
Filters.prototype.filterImages = function() {
  ...
  var border = new Float32Array(data.cells.length);
  for (var i=0; i<data.json.images.length; i++) {
    var keep = true;
    ...
    arr[i] = keep ? 1.0 : 0.20;
    border[i] = keep ? true : false
  }
  world.setBuffer('opacity', arr);
  world.setBuffer('selected', border);
}
...
function Tooltip() {
  ...
  this.targets = [
    ...
    },
    {
      ...
      {
      elem: document.querySelector('#layout-tsne'),
      text: 'Cluster images via TSNE dimensionality reduction',
    },
    {
...
function loadClient() {
    gapi.client.setApiKey("AIzaSyDqOIk4T8xDiUOhGU5wL3MxDD1lrvnZqJI");
    return gapi.client.load("https://www.googleapis.com/discovery/v1/apis/youtube/v3/rest")
        .then(function() { console.log("GAPI client loaded for API"); },
                function(err) { console.error("Error loading GAPI client for API", err); });
}
